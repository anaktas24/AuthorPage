...
// Updated Worldbuilding to include the map_url
interface Worldbuilding { magic: string; history: string; culture: string; map_url?: string; }
interface Book {
  id: number;
  title: string;
  description: string;
...
const BookDetail: React.FC = () => {
  ...
  // CHANGED: Split 'world' into 'map' and 'lore' to separate the views
  const [activeView, setActiveView] = useState<'synopsis' | 'characters' | 'map' | 'lore' | null>(null);
  ...
  const world = book.worldbuilding || { magic: '', history: '', culture: '', map_url: '' };

  // Logic for the Map Image
  const mapImage = world.map_url || "https://via.placeholder.com/1200x600?text=Interactive+World+Map";
        ...
        <div className="container mx-auto px-4 pt-32 pb-20 max-w-6xl">
          ...
          <div className="flex items-center justify-between mb-8">
            ...
            <h2 className="text-3xl font-header font-bold text-primary uppercase tracking-widest hidden md:block">
              ...
              {activeView === 'map' ? 'World Map' : activeView === 'lore' ? 'Lore & History' : activeView}
            </h2>
          </div>

          {/* CONTENT: SYNOPSIS (Exact Design Kept) */}
          {activeView === 'synopsis' && (
          ...
          {/* CONTENT: CHARACTERS (Drill Down View) */}
              ...
              {characters.length > 0 ? (
                characters.map((char) => (
                  <div
                    ...
                    className="bg-card shadow-card p-6 rounded-3xl flex flex-col items-center text-center border-t-4 border-accent transition-transform hover:-translate-y-2"
                  >

                     {/* IMAGE HANDLING: Image OR Initial */}
                     {char.image ? (
                        <img
                          src={char.image}
                          alt={char.name}
                          // Same sizing/border as your original code
                          className="w-32 h-32 rounded-full object-cover shadow-lg mb-4 border-4 border-white"
                        />
                     ) : (
                        <div className="w-32 h-32 rounded-full bg-gray-200 mb-4 flex items-center justify-center border-4 border-white shadow-lg">
                ...
                /* Empty State (Styled to match) */
                <div className="col-span-full bg-card shadow-card p-12 rounded-3xl text-center border border-gray-100">
                  <h3 className="text-2xl font-header font-bold text-primary mb-2">No Cast Added Yet</h3>
                  <p className="text-secondary font-body">Go to the Admin Panel to add characters.</p>
                </div>
              )}
            </div>
          )}

          {/* CONTENT: MAP & LOCATIONS (New Split View using your Design) */}
          {activeView === 'map' && (
             <div className="bg-card shadow-card rounded-3xl overflow-hidden">
                ...
                <div className="p-8 md:p-12">
                   <h4 className="font-bold text-accent uppercase tracking-wider mb-6">Locations</h4>
                   <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      ...
                      )) : <p className="text-secondary">No locations recorded yet.</p>}
                   </div>
                </div>
             </div>
          )}

          {/* CONTENT: LORE / MAGIC (New Split View using your Design) */}
          {activeView === 'lore' && (
             <div className="bg-card shadow-card rounded-3xl p-8 md:p-12 border border-gray-100">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                   <div className="md:col-span-2">
                      <h4 className="font-bold text-primary text-xl mb-2">Magic System</h4>
                      <p className="text-secondary leading-relaxed whitespace-pre-line">
                        {world.magic || "No magic system defined."}
                      </p>
                   </div>
                   ...
                   <div>
                      ...
                      <p className="text-secondary leading-relaxed whitespace-pre-line">
                        {world.history || "No history defined."}
                      </p>
                   </div>
                   ...
                   <div>
                      ...
                      <p className="text-secondary leading-relaxed whitespace-pre-line">
                        {world.culture || "No culture defined."}
                      </p>
                   </div>
                </div>
             </div>
      ...
      <div className="container mx-auto max-w-7xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 auto-rows-[200px]">
          ...
          <div
            ...
            className="md:col-span-1 md:row-span-2 bg-white text-black rounded-3xl p-6 cursor-pointer group hover:scale-[1.02] transition-transform relative overflow-hidden"
          >
          ...
          {/* TILE 3: WORLD MAP (UPDATED CLICK HANDLER) */}
          <div
             onClick={() => setActiveView('map')} // Now opens Map View
             className="md:col-span-1 bg-white rounded-3xl overflow-hidden relative cursor-pointer group"
          >
             <img src={mapImage} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
          ...
          {/* TILE 4: MAGIC/LORE (UPDATED CLICK HANDLER) */}
          <div
             onClick={() => setActiveView('lore')} // Now opens Lore View
