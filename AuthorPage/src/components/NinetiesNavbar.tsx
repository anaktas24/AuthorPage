...
const navItems = [
  { name: 'Home', path: '/', icon: 'https://media.giphy.com/media/3o7btPCcdNniyf0ArS/giphy.gif' },
  ...
  { name: "90's Look", path: '/nineties-look', icon: 'https://media.giphy.com/media/l0MYt5jPR6QX5pnqM/giphy.gif' },
];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col sm:flex-row justify-between items-center py-4">
          <div className="flex items-center mb-4 sm:mb-0">
            <h1 className="text-3xl text-[#FFFF00] animate-blink">Author's Rad Site</h1>
            <img
          ...
          <div className="flex flex-wrap items-center gap-2">
            {navItems.map((item) => (
              <Link
                key={item.name}
                to={item.path}
                className="nineties-nav-link flex items-center px-3 py-1 border-2 border-[#00FF00] bg-[#FF00FF] hover:bg-[#00FFFF] transition-colors duration-200"
              >
                <img src={item.icon} alt="Icon" className="w-5 h-5 mr-1" />
                {item.name}
              </Link>
            ))}
            <form onSubmit={handleSearch} className="flex items-center mt-2 sm:mt-0">
              <input
                type="text"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                placeholder="Search the Net!"
                className="border-2 border-[#FFFF00] bg-[#0000FF] text-[#00FF00] px-2 py-1 font-['VT323'] text-lg focus:outline-none"
              />
              <button
                type="submit"
